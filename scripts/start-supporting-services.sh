...
# If registry-bridge.sh already started a registry named 'local-registry' (port 5001), reuse it.
if docker ps --format '{{.Names}}' | grep -q '^local-registry$'; then
    echo "✅ local-registry already running (reused)"
else
    ...
    start_or_reuse_container "local-registry" "registry:2" "--network devcontainer-network -p 5001:5000 -e REGISTRY_STORAGE_FILESYSTEM_ROOTDIRECTORY=/data -v devcontainer_local_registry_data:/data"
fi
...
echo "   - Registry: local-registry:5000 (container access)"
