...
# Optionally unregister runners if removing data
...
# Optionally remove data volumes and local SSH config
if [ "$REMOVE_DATA" = true ]; then
    echo "⚠️  Removing GitLab data volumes (projects, configs, logs)..."
    docker volume rm gitlab_config gitlab_data gitlab_logs gitlab_runner_config 2>/dev/null || echo "Some volumes may not exist"
    ...
    echo "✅ All GitLab data and local configuration removed"
else
    echo "💾 GitLab data volumes and SSH configuration preserved (use --remove-data to remove)"
fi
