...
envsubst '${PROXY_PORT} ${BUCKET_NAME} ${BACKEND_CONTAINER_NAME} ${API_GATEWAY_URL} ${API_GATEWAY_HOSTNAME} ${API_GATEWAY_STAGE}' < /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf
